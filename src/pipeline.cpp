...
Pipeline::Pipeline(VkEngineDevice &device, const std::string &vertFilepath,
...
std::vector<char> Pipeline::readFile(const std::string &filePath) {
  ...
  if (!file.is_open()) {
    throw std::runtime_error("Falied to open file: " + enginePath);
  }
...
Pipeline::~Pipeline() {
  vkDestroyShaderModule(vkEngineDevice.device(), vertShaderModule, nullptr);
  vkDestroyShaderModule(vkEngineDevice.device(), fragShaderModule, nullptr);
  vkDestroyPipeline(vkEngineDevice.device(), graphicsPipeline, nullptr);
}
...
void Pipeline::createGraphicsPipeline(const std::string &vertFilepath,
  ...
  if (vkCreateGraphicsPipelines(vkEngineDevice.device(), VK_NULL_HANDLE, 1,
...
void Pipeline::createShaderModule(const std::vector<char> &code,
  ...
  if (vkCreateShaderModule(vkEngineDevice.device(), &createInfo, nullptr,
